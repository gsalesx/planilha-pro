...
router.post('/images/:orderId/:col', requireAuth, upload.single('image'), (req, res) => {
  ...
  const txn = db.transaction(() => {
    const existing = db
      .prepare('SELECT storage_path FROM images WHERE order_id = ? AND col = ?')
      .get(orderId, col) as { storage_path: string } | undefined
    if (existing) {
      try {
    ...
    db.prepare(
      `INSERT INTO images (order_id, col, file_name, mime, storage_path, updated_at)
       VALUES (?, ?, ?, ?, ?, ?)
       ON CONFLICT(order_id, col) DO UPDATE SET
         file_name = excluded.file_name,
         mime = excluded.mime,
         storage_path = excluded.storage_path,
         updated_at = excluded.updated_at`,
    ).run(orderId, col, req.file!.originalname || fileName, req.file!.mimetype, storagePath, now)
  ...
  res.json({
    ok: true,
    url: `/api/images/${encodeURIComponent(orderId)}/${col}`,
    updatedAt: now,
  })
})

router.delete('/images/:orderId/:col', requireAuth, (req, res) => {
  const orderId = req.params.orderId
  const col = Number(req.params.col)
  const existing = db
    .prepare('SELECT storage_path FROM images WHERE order_id = ? AND col = ?')
  ...
  const txn = db.transaction(() => {
    ...
    db.prepare('DELETE FROM images WHERE order_id = ? AND col = ?').run(orderId, col)
    db.prepare('UPDATE workbook_meta SET updated_at = ? WHERE id = 1').run(now)
  })
  txn()
...
router.get('/images/:orderId/:col', requireAuth, (req, res) => {
  const orderId = req.params.orderId
  const col = Number(req.params.col)
  const row = db
    .prepare('SELECT storage_path, mime FROM images WHERE order_id = ? AND col = ?')
    .get(orderId, col) as { storage_path: string; mime: string } | undefined
