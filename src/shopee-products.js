...
async function saveProductSku(itemId, sku) {
...
async function saveProductSkusSequential(updates, onProgress) {
    const results = [];
    for (let i = 0; i < updates.length; i++) {
        ...
        try {
            ...
            results.push({ itemId, ok: false, error: error.message });
        }
...
function formatPrice(value, hasModel) {
    if (value == null)
        return '—';
...
function formatStock(value) {
    if (value == null)
        return '—';
    if (value >= 1000)
        return `Estoque ${(value / 1000).toFixed(1).replace(/\.0$/, '')}k`;
...
async function runBulkSkuSave(itemIds, sku, setProgress) {
...
async function boot() {
  ...
  `;
    ...
    const bulkCountEl = root.querySelector('#bulk-count');
    const bulkSkuInput = root.querySelector('#bulk-sku-input');
    const bulkSaveBtn = root.querySelector('#btn-bulk-save');
    const progressWrap = root.querySelector('#bulk-progress');
    const progressBar = root.querySelector('#bulk-progress-bar');
    const progressText = root.querySelector('#bulk-progress-text');
    ...
    function deselectItems(itemIds) {
        for (const id of itemIds)
            selected.delete(id);
        const idSet = new Set(itemIds);
        gridEl.querySelectorAll('.product-select').forEach((cb) => {
            if (idSet.has(Number(cb.dataset.id)))
                cb.checked = false;
        });
    ...
    function bindGrid() {
        ...
        gridEl.querySelectorAll('.shopee-product-edit').forEach((btn) => {
        ...
        if (sortMode === 'price-asc' || sortMode === 'price-desc') {
            ...
            list = [...list].sort((a, b) => {
                if (a.price == null && b.price == null)
                    return 0;
                if (a.price == null)
                    return 1; // sem preço sempre por último
                if (b.price == null)
                    return -1;
    ...
    async function loadCatalog() {
        ...
        updateBulkBar();
        try {
            const data = await api('/shopee/products/catalog');
    ...
    selectAllEl.addEventListener('change', () => {
        ...
        if (selectAllEl.checked) {
            for (const p of visibleProducts)
                selected.add(p.itemId);
        }
        else {
            for (const p of visibleProducts)
                selected.delete(p.itemId);
        }
        gridEl.querySelectorAll('.product-select').forEach((cb) => {
    ...
    async function applyDaysToShipOne(itemId, days) {
        await api('/shopee/products/days-to-ship', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ itemId, daysToShip: days }),
        });
    }
    const daysToShipInput = root.querySelector('#days-to-ship-input');
    const daysToShipBtn = root.querySelector('#btn-days-to-ship');
    const daysToShipProgress = root.querySelector('#days-to-ship-progress');
    const daysToShipProgressBar = root.querySelector('#days-to-ship-progress-bar');
    const daysToShipProgressText = root.querySelector('#days-to-ship-progress-text');
    function setDaysToShipProgress(done, total, label) {
            ...
            onConfirm: async () => {
                ...
                for (let i = 0; i < total; i++) {
                    ...
                    try {
                        ...
                        console.warn('[days-to-ship] falhou', product.itemId, error.message);
                    }
                }
