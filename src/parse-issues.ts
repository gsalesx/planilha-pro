...
async function boot(): Promise<void> {
  ...
  const rowsEl = root.querySelector('#rows') as HTMLTableSectionElement
  const scanBtn = root.querySelector('#btn-scan') as HTMLButtonElement

  function renderRow(issue: ParseIssue): string {
    return `
      <tr data-id="${issue.id}">
  ...
  function bindRows(): void {
    rowsEl.querySelectorAll<HTMLButtonElement>('.parse-issue-resolve').forEach((btn) => {
      btn.addEventListener('click', async () => {
        ...
        try {
          ...
          if (!rowsEl.children.length) statusEl.textContent = 'Nenhuma pendência aberta.'
        } catch (error) {
  ...
  async function loadIssues(): Promise<void> {
    ...
    try {
      const data = await api<{ issues: ParseIssue[] }>('/parse-issues?resolved=0')
  ...
  scanBtn.addEventListener('click', async () => {
    ...
    try {
      const data = await api<{ issuesCreated: number }>('/parse-issues/scan', { method: 'POST' })
      await loadIssues()
      if (data.issuesCreated) alert(`${data.issuesCreated} pendência(s) nova(s) encontrada(s).`)
    } catch (error) {
      alert((error as Error).message)
    } finally {
