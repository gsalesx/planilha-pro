import { AuthRequiredError, deleteWorkbook, duplicateWorkbook, listWorkbooks, renameWorkbook, } from './api';
...
export function showWorkbooksList(opts) {
  ...
  `;
    const grid = root.querySelector('#wb-grid');
    const newFileInput = root.querySelector('#wb-new-file');
    const logoutBtn = root.querySelector('#wb-logout-btn');
    ...
    function handleError(error, fallback) {
        ...
        console.error(error);
        alert(`${fallback}: ${error.message ?? 'erro desconhecido'}`);
    }
    ...
    function renderGrid(workbooks) {
      ...
      `;
            const emptyInput = root.querySelector('#wb-empty-new');
      ...
      `)
            .join('');
        grid.querySelectorAll('.workbook-card').forEach((card) => {
            const id = card.dataset.id;
            const wb = workbooks.find((w) => w.id === id);
            card.querySelector('.wb-open').addEventListener('click', () => onOpen(id));
            const renameBtn = card.querySelector('.wb-rename');
            renameBtn?.addEventListener('click', () => openPromptDialog({
            ...
            card.querySelector('.wb-duplicate').addEventListener('click', () => openPromptDialog({
            ...
            const deleteBtn = card.querySelector('.wb-delete');
