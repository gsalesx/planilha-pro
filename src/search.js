...
export function searchWorkbook(workbook, query, maxResults = DEFAULT_MAX) {
    ...
    for (const sheetId of workbook.sheetOrder) {
        ...
        const positionByGlobal = new Array(sheet.rows.length).fill(0);
...
export function formatHitRef(hit) {
    if (hit.type === 'header')
        return `${colLetter(hit.colIndex)}1 · cabeçalho`;
...
export function highlightMatch(value, query, maxLength = 80) {
    ...
    if (html.length > maxLength * 6)
        html = html.slice(0, maxLength * 6) + '…';
    return html;
}
