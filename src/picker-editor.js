 ...
 */
export async function abrirPickerFila(itens, onSalvo) {
    for (let i = 0; i < itens.length; i++) {
        ...
        if (r === 'cancelado')
            return; // operador fechou: não segue a fila
    }
}
/** Tamanho da foto rotacionada-expandida — precisa bater com o `expand` do PIL. */
function tamanhoRotacionado(w, h, graus) {
...
export async function abrirPickerEditor(opts) {
    const { pieceId, slot } = opts;
    const base = `/api/pieces/${pieceId}/photo/${slot}`;
    const estado = await api(`${base}/ajuste`);
  ...
  `;
    ...
    const canvas = q('.picker-editor-canvas');
    const ctx = canvas.getContext('2d');
    const saida = q('.picker-editor-canvas--saida');
    const ctxSaida = saida.getContext('2d');
    const status = q('.picker-editor-status');
    const inZoom = q('.in-zoom');
    const inUWidth = q('.in-uwidth');
    const inBrush = q('.in-brush');
     ...
     */
    function anelDaMascara(m, cor, espessura) {
        ...
        for (const [ox, oy] of desloc)
            c.drawImage(m, ox, oy, CANVAS, CANVAS);
    ...
    async function carregarFonte() {
        ...
        try {
            ...
            if (modo === 'recorte') {
                ...
                q('.in-removebg').classList.add('destaque');
            }
            else {
    ...
    function desenharGuia(c) {
        ...
        if (modo === 'recorte') {
            ...
            if (heartAnel)
                c.drawImage(heartAnel, 0, 0);
        }
        c.restore();
    }
    /** Bolinha da borracha: mostra exatamente a área que o clique vai apagar. */
    function desenharCursorBorracha(c) {
        if (!borrachaAtiva || !cursor)
            return;
        c.save();
        c.beginPath();
    ...
    function desenhar() {
        ...
        if (modo === 'coracao') {
            if (heartMask)
                ctxSaida.drawImage(heartMask, 0, 0, CANVAS, CANVAS);
        }
        else {
    ...
    function caminhoCapsula(c, largura) {
        ...
        if (typeof c.roundRect === 'function')
            c.roundRect(x, 36, w, 864 - 36, r);
        else
            c.rect(x, 36, w, 864 - 36);
        c.closePath();
    }
    function desenharCapsula(c, largura) {
        c.save();
        c.fillStyle = '#fff';
    ...
    function sincronizarControles() {
        ...
        overlay.querySelectorAll('[data-so-recorte]').forEach((el) => {
    ...
    overlay.querySelectorAll('[data-rot]').forEach((b) => {
        b.addEventListener('click', () => {
    ...
    q('.in-borracha').addEventListener('click', (ev) => {
        borrachaAtiva = !borrachaAtiva;
        ev.currentTarget.classList.toggle('ativo', borrachaAtiva);
        canvas.style.cursor = borrachaAtiva ? 'none' : 'grab';
        if (!borrachaAtiva)
            cursor = null;
        desenhar();
    ...
    q('.in-removebg').addEventListener('click', async (ev) => {
    ...
    q('.in-reset').addEventListener('click', () => {
        ajuste.dx = 0;
        ajuste.dy = 0;
        ajuste.rotation = 0;
        ajuste.width = 0;
    ...
    async function aplicarBorracha() {
        if (!apagador || !fonte)
            return;
    ...
    q('.in-salvar').addEventListener('click', async (ev) => {
        const btn = ev.currentTarget;
        btn.disabled = true;
        setStatus('Salvando…');
        try {
            if (modo === 'recorte' && apagador)
                await aplicarBorracha();
    ...
    function fechar(resultado = 'cancelado') {
        overlay.remove();
    ...
    q('.picker-editor-close').addEventListener('click', () => fechar());
