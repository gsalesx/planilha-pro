import { addOrderPiece, assignPiecePhoto, confirmPiecesForOrder, copyPieceFrom, createCustomEmoji, deleteOrderPiece, fetchShopeeChatHistory, getEmojiCatalog, getOrderPieces, patchOrderDelta, removePiecePhoto, sendShopeeChatMessage, setPiecePhotoCrop, updateEmojiAliases, updateOrderPiece, uploadPiecePhoto, } from './api';
import { openConfirmDialog } from './dialog';
import { STATUS_COLUMN_INDEX } from './status';
import { openImageLightbox } from './lightbox';
import { abrirPickerEditor, abrirPickerFila } from './picker-editor';
...
const TAMANHO_OPTIONS = ['P', 'M', 'G', 'GG'];
function fmtMessageTime(ms) {
    if (!ms)
        return '';
    try {
...
function resolveEmojiText(text) {
    ...
    for (const item of emojiCatalog) {
        for (const alias of item.aliases) {
            if (alias && trimmed.includes(alias))
                return item;
        }
    }
    return null;
...
function catalogItemForCurrent(current) {
    if (!current)
        return null;
...
function pickFavorites() {
    ...
    return PRIORITY_EMOJI_NAMES.map((name) => byName.get(name)).filter((i) => !!i);
}
function emojiPickerHtml(pieceId, slot, current) {
...
function colorPickerHtml(pieceId, current) {
    const cur = (current || '#000000').toLowerCase();
    const swatches = SHORT_COLORS.map((c) => `<button type="button" class="color-swatch${c === cur ? ' is-selected' : ''}"
                    style="background:${c}" data-piece-id="${pieceId}" data-color="${c}" title="${c}"></button>`).join('');
...
function reloadChatImage(img, bumpRetry = false) {
...
function wireChatImages(root) {
    root.querySelectorAll('img.shopee-chat-image').forEach((img) => {
        if (img.dataset.wired === '1')
            return;
        img.dataset.wired = '1';
...
function renderMessages(messages, buyerUsername) {
...
export async function openShopeeChatPanel(order) {
  ...
  `;
    ...
    activePanel = overlay;
    const messagesEl = overlay.querySelector('#shopee-chat-messages');
    const inputEl = overlay.querySelector('.shopee-chat-input');
    const sendBtn = overlay.querySelector('.shopee-chat-send');
    const piecesEl = overlay.querySelector('#shopee-chat-pieces');
    const piecesOverlayEl = overlay.querySelector('#shopee-chat-pieces-overlay');
    const piecesToggleBtn = overlay.querySelector('#shopee-chat-pieces-toggle');
    const piecesToggleLabel = overlay.querySelector('#shopee-chat-pieces-toggle-label');
    const piecesCloseBtn = overlay.querySelector('#shopee-chat-pieces-close');
    const productPhotoBtn = overlay.querySelector('#shopee-chat-product-photo');
    productPhotoBtn?.addEventListener('click', () => {
        if (order.productImageUrl)
            openImageLightbox(order.productImageUrl, `Anúncio — ${order.product || order.orderId}`);
    });
    ...
    function renderPieceButtons() {
        piecesEl.querySelectorAll('.shopee-chat-piece-photo-pick').forEach((btn) => {
    ...
    function pieceCardHtml(piece, firstPieceId) {
        ...
        const generoOpts = ['MASCULINO', 'FEMININO']
            ...
            .join('');
        const tipoOpts = TIPO_OPTIONS.map((t) => `<option value="${t.value}"${piece.tipo === t.value ? ' selected' : ''}>${t.label}</option>`).join('');
        const tamanhoOpts = TAMANHO_OPTIONS.map((t) => `<option value="${t}"${piece.tamanho === t ? ' selected' : ''}>${t}</option>`).join('');
        function cropToggleHtml(slot) {
    ...
    function bindPieceCards() {
        piecesEl.querySelectorAll('.shopee-chat-piece-field').forEach((el) => {
            el.addEventListener('change', () => {
                const card = el.closest('.shopee-chat-piece-card');
                const pieceId = Number(card.dataset.pieceId);
                const field = el.dataset.field;
                const value = el.value;
                const patch = field === 'tipo'
                    ? { tipo: value }
                    : field === 'genero'
                        ? { genero: value }
                        : { tamanho: value };
        ...
        piecesEl.querySelectorAll('.emoji-picker-fav').forEach((btn) => {
        ...
        piecesEl.querySelectorAll('.emoji-picker-gallery-btn').forEach((btn) => {
            btn.addEventListener('click', () => {
                openEmojiGallery(Number(btn.dataset.pieceId), (btn.dataset.slot === '1' ? 1 : 2));
            });
        });
        piecesEl.querySelectorAll('.emoji-picker-input').forEach((inp) => {
            inp.addEventListener('keydown', (e) => {
                if (e.key !== 'Enter')
                    return;
                e.preventDefault();
                const pieceId = Number(inp.dataset.pieceId);
                const slot = (inp.dataset.slot === '1' ? 1 : 2);
                const text = inp.value;
                inp.value = '';
                void resolveAndApplyEmoji(pieceId, slot, text);
            });
        });
        piecesEl.querySelectorAll('.color-swatch').forEach((btn) => {
            btn.addEventListener('click', () => {
                const pieceId = Number(btn.dataset.pieceId);
        ...
        piecesEl.querySelectorAll('.color-custom-input').forEach((inp) => {
            inp.addEventListener('change', () => {
                const pieceId = Number(inp.dataset.pieceId);
                void updateOrderPiece(pieceId, { cor: inp.value }).then(() => loadPieces());
            });
        });
        piecesEl.querySelectorAll('.shopee-chat-piece-nota-toggle').forEach((btn) => {
            btn.addEventListener('click', () => {
                const pieceId = btn.dataset.pieceId;
                const box = piecesEl.querySelector(`.shopee-chat-piece-nota[data-piece-id="${pieceId}"]`);
                if (!box)
                    return;
        ...
        piecesEl.querySelectorAll('.shopee-chat-piece-nota-input').forEach((ta) => {
        ...
        piecesEl.querySelectorAll('.shopee-chat-piece-photo-pick').forEach((btn) => {
            btn.addEventListener('click', () => {
                armSlot(Number(btn.dataset.pieceId), Number(btn.dataset.slot));
            });
        });
        piecesEl.querySelectorAll('.shopee-chat-piece-photo-upload-input').forEach((input) => {
            input.addEventListener('change', () => {
                void (async () => {
                    ...
                    try {
                        ...
                        alert(`Falha ao subir foto: ${error.message}`);
                        input.value = '';
                    }
                })();
            });
        });
        piecesEl.querySelectorAll('.shopee-chat-piece-photo-remove').forEach((btn) => {
            btn.addEventListener('click', async () => {
                await removePiecePhoto(Number(btn.dataset.pieceId), Number(btn.dataset.slot));
                void loadPieces();
            });
        });
        piecesEl.querySelectorAll('.shopee-chat-piece-ajustar').forEach((btn) => {
            btn.addEventListener('click', () => {
        ...
        piecesEl.querySelectorAll('.shopee-chat-piece-crop input[type=radio]').forEach((radio) => {
            radio.addEventListener('change', async () => {
                const pieceId = Number(radio.dataset.pieceId);
                const slot = Number(radio.dataset.slot);
                ...
                try {
                    await setPiecePhotoCrop(pieceId, slot, radio.value);
                }
                catch (error) {
                    alert(error.message);
                }
        ...
        piecesEl.querySelectorAll('.shopee-chat-piece-copy-first').forEach((btn) => {
            btn.addEventListener('click', async () => {
                const pieceId = Number(btn.dataset.pieceId);
        ...
        piecesEl.querySelectorAll('.shopee-chat-piece-delete').forEach((btn) => {
            btn.addEventListener('click', async () => {
    ...
    async function resolveAndApplyEmoji(pieceId, slot, text) {
        const trimmed = text.trim();
        if (!trimmed)
            return;
    ...
    function closeEmojiGallery() {
        document.getElementById('emoji-gallery-modal')?.remove();
    }
    function openEmojiGallery(pieceId, slot, opts = {}) {
        closeEmojiGallery();
        const field = slot === 1 ? 'emoji1' : 'emoji2';
    ...
    `;
        ...
        const gridEl = modal.querySelector('.emoji-gallery-grid');
        const searchEl = modal.querySelector('.emoji-gallery-search');
        const closeBtn = modal.querySelector('.emoji-gallery-close');
        const moldBtn = modal.querySelector('.emoji-gallery-mold-btn');
        const uploadInput = modal.querySelector('.emoji-gallery-upload-input');
        const uploadNameInput = modal.querySelector('.emoji-gallery-upload-name');
        ...
        async function escolher(item) {
            if (opts.pendingChar && !item.aliases.includes(opts.pendingChar)) {
                try {
                    ...
                    alert(`Não salvei o atalho "${opts.pendingChar}": ${error.message}`);
                }
            }
        ...
        function render(items) {
            ...
            gridEl.querySelectorAll('.emoji-gallery-item').forEach((btn) => {
                btn.addEventListener('click', () => {
        ...
        uploadInput.addEventListener('change', () => {
            void (async () => {
                ...
                try {
                    ...
                    alert(`Falha ao subir emoji: ${error.message}`);
                }
            })();
        });
    }
    function updatePiecesToggleLabel(data) {
    ...
    function confirmBarHtml(pieces) {
    ...
    function bindConfirmBar(pieces) {
        const btn = overlay.querySelector('#shopee-chat-confirm-order');
        if (!btn)
            return;
        btn.addEventListener('click', () => {
            const missing = pieces.filter((p) => !p.photos[1]).length;
     ...
     */
    function montarFilaAjuste(pieces) {
        const btn = overlay.querySelector('#shopee-chat-ajustar-todas');
        if (!btn)
            return;
        const fila = [];
    ...
    async function loadPieces() {
      ...
      `;
            ...
            piecesEl.innerHTML = `<div class="shopee-chat-error-inline">Falha ao carregar peças: ${escapeHtml(error.message)}</div>`;
            piecesToggleLabel.textContent = 'Peças da arte';
        }
    }
    messagesEl.addEventListener('click', (e) => {
        const retryBtn = e.target.closest('.shopee-chat-image-retry');
        if (retryBtn) {
            e.preventDefault();
            e.stopPropagation();
            const wrap = retryBtn.closest('.shopee-chat-image-wrap');
            const img = wrap?.querySelector('img.shopee-chat-image');
            if (img)
                reloadChatImage(img, false);
            return;
        }
        if (!armed)
            return;
        const link = e.target.closest('.shopee-chat-image-link');
        if (!link)
            return;
        e.preventDefault();
        ...
        void assignPiecePhoto(pieceId, slot, url)
            ...
            .catch((error) => {
            alert(`Falha ao usar essa foto: ${error.message}`);
        });
    });
    ...
    sendBtn.addEventListener('click', async () => {
        ...
        try {
            ...
            messagesEl.insertAdjacentHTML('beforeend', `<div class="shopee-chat-error-inline">Falha ao enviar: ${escapeHtml(error.message)}</div>`);
        }
        finally {
    ...
    try {
        ...
        if (history.truncated) {
            messagesEl.insertAdjacentHTML('afterbegin', `<div class="shopee-chat-warn">Histórico truncado (${history.pages} páginas). Mensagens mais antigas podem não aparecer.</div>`);
        }
        messagesEl.scrollTop = messagesEl.scrollHeight;
        syncSendState();
        inputEl.focus();
        ...
        messagesEl.innerHTML = `<div class="shopee-chat-error">${escapeHtml(error.message)}</div>`;
        sendBtn.disabled = true;
        inputEl.disabled = true;
    }
}
